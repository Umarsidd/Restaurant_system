...
class Command(BaseCommand):
    ...
    def assign_permissions(self, user, role):
        ...
        if role == 'WAITER':
            # Waiters can manage orders and view menu/tables
            permissions = [
                # Orders
                Permission.objects.get(codename='add_order', content_type__model='order'),
                Permission.objects.get(codename='change_order', content_type__model='order'),
                Permission.objects.get(codename='view_order', content_type__model='order'),
                # OrderItems
                Permission.objects.get(codename='add_orderitem', content_type__model='orderitem'),
                Permission.objects.get(codename='change_orderitem', content_type__model='orderitem'),
                Permission.objects.get(codename='view_orderitem', content_type__model='orderitem'),
                # Tables (view only)
                Permission.objects.get(codename='view_table', content_type__model='table'),
                # Menu (view only)
                Permission.objects.get(codename='view_menuitem', content_type__model='menuitem'),
            ]
            user.user_permissions.set(permissions)
            
        elif role == 'CASHIER':
            # Cashiers can manage bills and view orders/tables
            permissions = [
                # Bills
                Permission.objects.get(codename='add_bill', content_type__model='bill'),
                Permission.objects.get(codename='change_bill', content_type__model='bill'),
                Permission.objects.get(codename='view_bill', content_type__model='bill'),
                # Orders (view only)
                Permission.objects.get(codename='view_order', content_type__model='order'),
                Permission.objects.get(codename='view_orderitem', content_type__model='orderitem'),
                # Tables (view only)
    ...
    def handle(self, *args, **kwargs):
        ...
        if _:
            ...
            waiter1.save()
        # Assign waiter permissions
        self.assign_permissions(waiter1, 'WAITER')
        ...
        if _:
            ...
            waiter2.save()
        # Assign waiter permissions
        self.assign_permissions(waiter2, 'WAITER')
        ...
        self.assign_permissions(cashier, 'CASHIER')
